  ...
  if (setInning === 1) {
    ...
    if (team1Score > team2Score) {
      document.getElementById(
        "result"
      ).innerHTML = `<h2>${teamOneName} Won by ${result} runs</h2>`;
    } else if (team1Score < team2Score) {
      document.getElementById(
        "result"
      ).innerHTML = `<h2>${teamTwoName} Won by ${wicket} wicket</h2>`;
    } else {
...
function calculation(run) {
  if (!resetOver) {
    ...
    document.getElementById(
      "liveScore"
    ).innerHTML = `<p>Current Score: ${totalScore} Runs and ${totalWicket} wickets</p>`;
    ...
    if (totalBall === 6) {
      ...
      if (setInning === 1) {
        document.getElementById(
          "scoreHistory"
        ).innerHTML += `<p class="over-by-over"> ${teamOneName}, Over ${over}: ${overByOverScore} runs</p>`;
      } else {
        document.getElementById(
          "scoreHistory"
        ).innerHTML += `<p class="over-by-over"> ${teamTwoName}, Over ${over}: ${overByOverScore} runs</p>`;
      }
...
function totalScorePrint() {
  if (flag === true && setInning === 1) {
    ...
    document.getElementById(
      "totalRun"
    ).innerHTML += `${teamOneName} Total Score: ${totalScore} <br>`;
    flag = false;
  } else if (flag === true && setInning === 2) {
    team2Score = totalScore;
    document.getElementById(
      "totalRun"
    ).innerHTML += `${teamTwoName} Total Score: ${totalScore}`;
    flag = false;
  }
}
